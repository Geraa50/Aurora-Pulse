...
## passable = true → `mouse_filter = IGNORE`, плитка вообще не получает
## input-события. Board выставляет это любому НЕсвободному тайлу: клик по
## области, где сверху что-то лежит, должен проваливаться сквозь нижние плитки
## и попадать только в реально свободную, видимую сверху плитку.
